...
const questions = [
  ...
  { id: 11, text: "Which is the outermost meningeal layer?", options: ["Arachnoid mater", "Dura mater", "Pia mater", "Epineurium"], correctIndex: 1 },
  ...
  { id: 13, text: "The meningeal layer that closely follows contours of the brain and contains many blood vessels is the:", options: ["Dura mater", "Arachnoid mater", "Pia mater", "Endoneurium"], correctIndex: 2 },
  ...
  { id: 29, text: "The white matter regions are known as:", options: ["Horns", "Funiculi (columns)", "Meninges", "Ganglia"], correctIndex: 1 },
  { id: 30, text: "Bundles of axons in white matter carrying information up or down are called:", options: ["Tracts", "Nuclei", "Ganglia", "Nodes"], correctIndex: 0 },
  ...
  { id: 32, text: "Which root carries sensory impulses into the spinal cord?", options: ["Ventral root", "Dorsal root", "Lateral root", "Median root"], correctIndex: 1 },
  { id: 33, text: "Which root carries motor impulses out of the spinal cord?", options: ["Ventral root", "Dorsal root", "Posterior root", "Ganglionic root"], correctIndex: 0 },
  ...
  { id: 36, text: "The connective tissue layer surrounding an entire nerve is the:", options: ["Endoneurium", "Perineurium", "Epineurium", "Myelin sheath"], correctIndex: 2 },
  ...
  { id: 42, text: "A pathway from receptors via sensory neurons to CNS then to effectors is called a:", options: ["Corticospinal tract", "Reflex arc", "Pyramidal tract", "Extrapyramidal pathway"], correctIndex: 1 },
  { id: 43, text: "Ascending tracts primarily carry:", options: ["Motor impulses to muscles", "Sensory information to the brain", "Autonomic signals to glands", "CSF between ventricles"], correctIndex: 1 },
  { id: 44, text: "Descending tracts primarily carry:", options: ["Sensory information to the brain", "Pain only", "Motor commands from brain to effectors", "Reflexes to the skin"], correctIndex: 2 },
  ...
  { id: 46, text: "In the patellar reflex the sensory neuron synapses with:", options: ["Another sensory neuron", "A motor neuron in the spinal cord", "A neuron in the brainstem only", "A skeletal muscle cell"], correctIndex: 1 },
...
type QMap = { [key: number]: number };

export default function Home() {
  const [selected, setSelected] = React.useState<QMap>({});
  const [submitted, setSubmitted] = React.useState<boolean>(false);

  const pick = (qi: number, oi: number): void => {
    if (submitted) return;
    setSelected((prev: QMap) => ({ ...prev, [qi]: oi }));
  };

  const score: number = questions.filter((q, i) => selected[i] === q.correctIndex).length;
  ...
  return (
    <div style={{maxWidth:800,margin:"0 auto",padding:"20px",fontFamily:"Arial,sans-serif"}}>
      ...
      <p style={{textAlign:"center",color:"#555",marginBottom:"30px"}}>Grade 12 Anatomy - 50 Questions</p>
      {questions.map((q, qi) => (
        <div key={qi} style={{border:"1px solid #ccc",borderRadius:"8px",padding:"16px",marginBottom:"16px",background:"#fafafa"}}>
          ...
          {q.options.map((opt, oi) => {
            const isSel: boolean = selected[qi] === oi;
            const isRight: boolean = q.correctIndex === oi;
            let bg: string = "#fff";
            let border: string = "1px solid #ccc";
            ...
            return (
              <div key={oi} onClick={() => pick(qi, oi)} style={{padding:"10px 14px",marginBottom:"6px",borderRadius:"6px",border:border,background:bg,cursor:submitted?"default":"pointer",fontSize:"15px",userSelect:"none"}}>
